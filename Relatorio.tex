...
No experimento mostrado na tabela \ref{tab:lump_elmore_slew}, o modelo de capacitância concentrada foi utilizado para modelar a carga vista pelo \textit{driver}. Para o atraso das interconexões, a técnica de Elmore foi utilizada\footnote{O atraso de Elmore foi multiplicado pelo fator de \textit{scaling} $ln(2)$. Assim, o atraso de Elmore representa a resposta ao impulso ao realizar metade da transição completa do sinal \citacaoAzul{Rabaey08}.}. E para a degradação do \textit{slew}, foi utilizada a técnica descrita no capítulo \ref{cap:tecnica}. Como esperado, os modelos utilizados neste experimento refletem em uma aproximação pessimista para o atraso do circuito, já que a técnica de Elmore pura\footnote{Sem utilizar a abordagem de capacitância efetiva.} foi aplicada no cálculo dos atrasos das interconexões.
...
O experimento apresentado na tabela \ref{tab:ceff_ctotal_todos} mostra que nos circuitos da competição de \textit{sizing} do ISPD, o valor de $C_{eff}$ médio é muito próximo do valor de $C_{total}$. Este experimento explica o baixo erro obtido ao utilizar-se o modelo de capacitância concentrada para estimar o valor da capacitância vista pelo \textit{driver} (tabela \ref{tab:lump_elmore_slew}). A tabela \ref{tab:ceff_ctotal_todos} mostra as relações $C_{eff}/C_{total}$ médias para todos os circuitos, os considerando em três configurações diferentes:
	\begin{itemize}
