...
\chapter{Técnica de \citacaoInline{PURI02} para o Cálculo das Características Temporais da Interconexão}
\label{cap:tecnica}
...
Por fim, o experimento da tabela \ref{tab:ceff_elmore_no_slew} utiliza a abordagem de capacitância efetiva, como apresentada no capítulo \ref{cap:tecnica}, porém, sem considerar a degradação do \textit{slew}. Os erros obtidos neste experimento (-33.11\% para TNS e 19.99\% para critical path) mostram resultados muito otimistas em relação ao \textit{PrimeTime}, destacando a importância de se considerar a degradação do \textit{slew} nas interconexões.
