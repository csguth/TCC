...
\titulo{Análise de \emph{Timing} Estática e o Impacto dos Atrasos das Interconexões em Cicuitos Digitais}
...
\textoResumo {
Análise de \emph{Timing} Estática (\emph{Static Timing Analysis}) é uma das técnicas utilizadas para estimar as informações temporais em projetos de circuitos digitais. A medida em que o projeto se aproxima da fase de \emph{layout}, a Análise de \emph{Timing} se torna cada vez mais importante, sendo utilizada em diversas etapas do fluxo de projeto. Um dos problemas no fluxo de projeto de circuitos digitais é a informação precisa de \emph{timing} das células dos circuitos e das interconexões, sendo que muitas técnicas de análise de \emph{timing}, consideram apenas, modelos de atrasos muito simplificados, ou até, desconsideram a importância dos atrasos das interconexões. Este trabalho tem por objetivo o projeto, a implementação, a validação e a avaliação de uma técnica de análise de \emph{timing} que leva em consideração os atrasos das interconexões.
}
\palavrasChave {automatização do projeto eletrônica, gate sizing, \emph{CMOS}, programação dinâmica, relaxação
lagrangeana, fluxo de projeto \emph{standard cell}}
...
\chapter{Introduç\~ao}
	...
	\section{Motivação}
		...
		O objetivo, tanto dos usuários, quanto dos fabricantes, é maximizar o ciclo de vida das baterias, ainda que mais recursos, de \emph{software} ou \emph{hardware}, sejam integrados em um mesmo dispositivo. Entretanto, a capacidade de armazenamento de energia das baterias não tem evoluido ao mesmo passo em que essa integração acontece. Como pode ser visualizado na Figura \ref{fig:batteryTrends}, desde 2003, com o advento dos \emph{Smartphones} e aplicações multimídia móveis, há um \emph{gap} de $1W$ entre o consumo de potência dos circuitos digitais dessa natureza e a capacidade de suas baterias, demandando o emprego de técnicas de projeto de baixo consumo por parte dos projetistas.
	...
	\section{O Fluxo de Projeto \emph{Standard Cell}}
		...
		% \includegraphics{sta.png}
		\caption{Fluxo de projeto CMOS \cite{BhaskerChadha09} \afazer{REFERENCIAR A IMAGEM E EXPLICAR}}
		\label{fig:cmos_flow}
		\end{center}
		\end{figure}
	...
	\section{Objetivos}
		...
		\subsection{Objetivos Específicos}
			...
			\begin{enumerate}
				...
				\item Implementar o modelo de atraso de interconexões Elmore \cite{Elmore48}; \label{objetivos:elmore}

				\item Implementar o modelo de atraso de interconexões utilizando a abordagem de Capacitância Efetiva, como descrito em \cite{PURI02}; \label{objetivos:puri};

				\item Comparação e avaliação dos métodos descritos nos objetivos \ref{objetivos:elmore} e \ref{objetivos:puri} com ferramenta comercial de Análise de \emph{Timing} Synopsys PrimeTime \textregistered \cite{PrimeTime12};
		...
		\section{Escopo}
			...
			A ferramenta considerará também, os atrasos de interconexões, utilizando pelo menos, 3 modelos de atraso, Elmore \cite{Elmore48}, abordagem Capacitância Efetiva \cite{PURI02} e modelo de Capacitância Concentrada \cite{Rabaey08}.

			Por fim, a ferramenta será validada perante à ferramenta comercial Synopsys PrimeTime\textregistered \cite{PrimeTime12} e incorporada a uma ferramenta de \emph{Gate Sizing} \cite{Livramento13}.
			...
			\item Uma ferramenta de análise de \emph{timing} estática Synopsys PrimeTime \textregistered \cite{PrimeTime12}  para comparação de resultados;
			\end{itemize}
...
\chapter{Modelo de Interconexões}
	...
	\section{Modelo RC Concentrado (\emph{Lumped RC Model})}
		...
		Uma das métricas mais populares para se calcular o atraso em interconexões é o atraso de Elmore (\emph{Elmore Delay}) \cite{Elmore48}, pela simplicidade e razoável correlação com os atrasos reais. Em um nodo $i$ da Árvore RC, o atraso de Elmore ($\tau_{Di}$) pode ser facilmente calculado como:
		\begin{equation}
	...
	\section{Modelo RC Distribuído (\emph{Distributed RC Model})}
		...
		Em \cite{Kashyap00}, é proposta uma métrica, que leva em consideração o efeito de \emph{resistive shield}, calculando uma Capacitância Efetiva ($C_{eff}$) para modelar o impacto das capacitâncias \emph{downstream}.

		Devido a limitação de que o cálculo da Capacitância Efetiva depende do \emph{slew} aplicado no nodo fonte da Árvore RC, e que por sua vez, depende da Capacitância Efetiva, este trabalho abordará uma alternativa, proposta em \cite{PURI02}, computacionalmente eficiente e precisa, para estimativa dos atrasos das interconexões em circuitos \emph{CMOS}, contornando o problema observado em \cite{Kashyap00}.
...
\chapter{An\'alise de \emph{Timing} Estática}
	...
	% Análise de \emph{Timing}, ou \emph{Timing Analysis} \cite{Guntzel00} \cite{BhaskerChadha09}, é uma das técnicas utilizadas para se estimar atrasos das células e interconexões em circuitos digitais. A Análise de \emph{Timing} é chamada de estática (STA), quando não depende dos valores aplicados nas entradas primárias do circuito, ou seja, apenas considera o comportamento estático das portas lógicas e interconexões. É um processo completo e exaustivo \cite{BhaskerChadha09} que verifica as mais diversas informações de \emph{timing} em um circuito, como por exemplo, atrasos e \emph{slew} (tempo de transição) pino-a-pino, atrasos em interconexões, folgas e \emph{required times}.

	% Dado um design de entrada, restrições de projeto e uma biblioteca de células, o objetivo da Análise de \emph{Timing} é validar se o \emph{design} em análise (\emph{DUA}) pode operar no desempenho estipulado. Ou seja, se o circuito final poderá funcionar, com segurança, na frequência de relógio escolhida, sem que existam violações de \emph{timing}.

	% \begin{figure}[ht]
	% \begin{center}
	% \includegraphics[scale=0.25]{imagens/sta.png}
	% % \includegraphics{sta.png}
	...
	% A lógica combinacional de um circuito, pode ser representada como um grafo direcionado $G = (V, A)$, onde V é o conjunto das portas lógicas, entradas e saídas primárias. Um par de vértices, ou portas lógicas, $v_i$ e $v_j \in V$, são conectadas por uma aresta direcionada $(v_i,v_j) \in A$ se existe uma interconexão da saída de $v_i$ para uma entrada de $v_j$ \cite{Sapatnekar04}. Um modelo de circuito combinacional e seu grafo correspondente podem ser visualizados na Figura \ref{fig:circuit_graph}.
...
\chapter{Calculo do Atraso das Interconexões}

	\section{Técnica de Elmore}
	\afazer{A fazer}

	\section{Técnica de \citeonline{PURI02}}
	\afazer{A fazer, conforme em \cite{PURI02}}
...
\chapter{Experimentos}
	...
	\section{Validação da Ferramenta de STA Perante a uma Ferramenta Industrial}
		\afazer{A FAZER}

	\section{Aplicação da Ferramenta de STA em uma Técnica de \emph{Gate Sizing}}
		\afazer{A FAZER}
