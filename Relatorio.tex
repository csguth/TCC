...
\caption{Acima: Uma porta lógica \textit{CMOS} $u1$, de função \textbf{NAND}, com duas entradas é \textit{driver} da interconexão \textit{n1}. Abaixo: As características temporais (\textit{delay} e \textit{slew}) da porta lógica $u1$.}
...
Como parte dos experimentos é realizada comparando as informações calculadas pela ferramenta implementada com as informações reportadas pelo \textit{PrimeTime}, o erro relativo percentual foi adotado como métrica para a qualidade das informações de \textit{timing} reportadas pela ferramenta implementada neste trabalho. O $Erro\ Percentual$ (equação \ref{eq:erro_relativo}) de $a$ e $b$ é o valor percentual da relação entre erro absoluto de $a$ e $b$, e o valor do maior valor absoluto entre $a$ e $b$, valendo 100\% quando o erro é o maior possível ($a = 0$ ou $b = 0$), e 0\% quando o erro é o menor possível ($a = b$). O erro absoluto entre $a$ e $b$ é o valor absoluto $a - b$, denotado por $|a - b|$.
	...
	Como $a$ e $b$ são números em ponto flutuante, pode não ser possível computacionalmente, obter-se $a = b$, sendo necessário o uso de um valor $\varepsilon$ para a precisão. Assim, $a$ e $b$ são considerados iguais se o erro relativo percentual é menor ou igual a $\varepsilon$.
...
É possível observar que o \textit{runtime} da análise de \textit{timing} no \textit{PrimeTime} é muito alto, sendo inviável o seu uso em uma técnica de otimização que necessita avaliar as informações temporais milhares de vezes. Porém, devido à diferença na implementação dos modelos de atraso, os valores de \textit{TNS} obtidos na ferramenta implementada são bem maiores que os obtidos pelo \textit{PrimeTime}, mostrando que a ferramenta implementada é pessimista, ao tomar as soluções do \textit{PrimeTime} como sendo reais.
...
A consideração do atraso das interconexões no fluxo \textit{standard cell} é muito importante e a avaliação desses atrasos deve ser eficiente e precisa. A abordagem da capacitância efetiva para interconexões implica na consideração do efeito de \textit{resistive shielding}, o qual impacta na qualidade do cálculo do atraso das interconexões. A ferramenta implementada neste trabalho apresentou ser eficiente em termos de \textit{runtime}, obtendo resultados bons de análise de \textit{timing}, porém pessimistas comparados ao \textit{PrimeTime}.
