...
\chapter{An\'alise de \emph{Timing} Estática}
	...
	Dado um design de entrada, restrições de projeto e uma biblioteca de células (Figura \ref{fig:STA}), o objetivo da Análise de \emph{Timing} é validar se o \emph{design} em análise (\emph{DUA: Design Under Analysis}) pode operar no desempenho estipulado, ou seja, se o circuito final poderá funcionar, com segurança, na frequência de relógio escolhida, sem que existam violações nas restrições de projeto.
	...
	\afazer{
		...
		\begin{itemize}
			...
			\item O atraso das portas lógicas é dependente do modelo de interconexão, então, certas informações ficaram a cargo do capítulo \ref{cap:}
...
\chapter{Implementação do Atraso das Interconexões}
	\label{cap:atraso}
