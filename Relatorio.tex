...
Em diversos sistemas projetados atualmente, entre 50\% a 70\% do ciclo de relógio é ``consumido'' pelo atraso de propagação de suas interconexões \citacaoAzul{Cong96}. Nas tecnologias com alta escala de integração (\textit{VLSI: Very-large-scale integration}) atuais, onde diversas otimizações tem por objetivo reduzir a resistência dos \textit{drivers}, as interconexões passam a ser cada vez mais impactantes no desempenho do circuito digital.
...
Apesar da alta precisão das técnicas baseadas em momentos de alta ordem, como o \textit{AWE}, o custo computacional ainda é elevado, fazendo com que técnicas de momentos de ordem menor sejam utilizados.
