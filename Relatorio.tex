...
\includegraphics[width=\linewidth, trim=3cm 8cm 3cm 0, clip=true]{imagens/arco_de_timing_fall_rise_slew_delay.pdf}
...
\end{center}
\caption{(a) Interconexão RC obtida do circuito $simple$ da competição de \textit{sizing} do ISPD \cite{Contest2013}. (b) SPEF referente à Figura \ref{fig:spef_example}.}
\end{figure}
...
\caption{Formas de onda na saída de uma porta lógica em função da abordagem utilizada para cálculo da capacitância. Obtida de \cite{BhaskerChadha09}.}
...
\begin{figure}[ht!]
\begin{center} % TRIM = L D R U
\includegraphics[width=0.7 \linewidth]{imagens/circuito_exemplo_modelagem_interconnect.png} 
...
\begin{figure}[ht!]
\begin{center} % TRIM = L D R U
\includegraphics[width=0.7 \linewidth]{imagens/circuito_exemplo_modelagem_interconnect2.png} 
...
Esse capítulo tem por objetivo apresentar os experimentos realizados, bem como os resultados obtidos por estes. Como parte dos experimentos é realizada comparando as informações calculadas pela ferramenta implementada com as informações reportadas pelo \textit{PrimeTime}, a seção \ref{sec:erro} apresentará a métrica para cálculo dos erros que foi utilizada no presente trabalho. Na seção  \ref{sec:validacao_lumped}, é apresentado o primeiro experimento, que trata da validação da ferramenta perante o \textit{PrimeTime}, utilizando o modelo de interconexões de capacitância concentrada. Na seção \ref{sec:sta_industrial} será apresentada a maneira como o \textit{PrimeTime} modela o circuito, e como as informações de \textit{timing} são calculadas. A seção \ref{sec:validacao_ceff} tem por objetivo mostrar os resultados obtidos com a técnica de cálculo de capacitância efetiva e degradação do \textit{slew} implementadas neste trabalho. E por fim, serão apresentados os resultados obtidos utilizando algumas variações nos modelos de atraso.
	...
	Como $a$ e $b$ se tratam de números em ponto flutuante, não é possível computacionamente, obter se $a = b$, sendo necessário o uso de um valor $\epsilon$ para a precisão. Assim, $a$ e $b$ são considerados iguais se o erro relativo percentual é menor ou igual a $\epsilon$.
...
Por exemplo, supondo que $a = 3.12534$ e $b = 3.12566$, aplicando a equação \ref{eq:erro_relativo}, obtemos $Erro\ Percentual = 0.0102378\%$. Se $\epsilon = 0.1$, $a$ e $b$ são considerados iguais.
...
Esta seção tem por objetivo apresentar o modelo utilizado para cálculo do desempenho na ferramenta industrial \textit{PrimeTime} e apresentar algumas informações de \textit{timing} relevantes, obtidas para cada circuito da competição de \textit{sizing} do ISPD \cite{Contest2013}.

\begin{figure}[ht]
\begin{center}% TRIM = L D R U
\includegraphics[width=\textwidth]{imagens/modelo_primetime.png} 
\caption{Modelagem utilizada no \textit{PrimeTime} para o \textit{driver}, interconexão e destinos. Obtida de \cite{Ptug13}.}
...
De acordo com \cite{Ptug13}, o modelo de \textit{driver} tem três parâmetros: 
\begin{itemize}
...
\caption{Valores obtidos pelo \textit{PrimeTime} nos circuitos da competição de \textit{sizing} do ISPD \cite{Contest2013}.}
\label{tab:comparacoes_primetime}
\end{center}
\end{table}
...
Esta seção tem por objetivo, comparar a qualidade das informaçoes de \textit{timing} obtidas pela ferramenta de análise de \textit{timing} implementada neste trabalho com  as informações reportadas pelo \textit{PrimeTime}. Utilizando a técnica de cálculo de capacitância efetiva, atraso de interconexões e degradação de \textit{slew} apresentadas no capítulo \ref{cap:tecnica}, a análise de \textit{timing} estática foi aplicada nos seis circuitos da competição de \textit{sizing} do ISPD \cite{Contest2013}, e suas soluções foram comparadas com o \textit{PrimeTime} utilizando a métrica apresentada na seção \ref{eq:erro_relativo}.

A tabela \ref{tab:ceff_elmore_slew} mostra os valores obtidos pela ferramenta implementada neste trabalho, para cada um dos seis circuitos da competição de \textit{sizing} do ISPD \cite{Contest2013}. 

\begin{table}[ht]
\begin{center}% TRIM = L D R U
\includegraphics[width=\textwidth, trim=1.8cm 20.5cm 7cm 1cm, clip=true]{imagens/comparacoes/ceff_elmore_slew.pdf}
\caption{Valores obtidos pela ferramenta implementada neste trabalho nos circuitos da competição de \textit{sizing} do ISPD \cite{Contest2013}.}
