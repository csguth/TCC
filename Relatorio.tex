...
\chapter{An\'alise de \emph{Timing} Estática}
	...
	\section{Cálculo e Propagação dos Atrasos}
		...
		Nas bibliotecas de célula atuais são fornecidos modelos de atrasos para os arcos de \emph{timing} das células disponíveis, esses modelos, geralmente são obtidos através de simulações em nível elétrico.

		Considerando o inversor da Figura \ref{fig:arco_de_timing_fall_rise_slew_delay}, seu arco de \emph{timing} é representado pela conexão entre o pino de entrada $a$ e o pino de saída $o$, o atraso e o tempo de propagação em seu arco de \emph{timing} são funções não lineares depentendes de dois fatores, capacitância total vista no pino de saída da porta lógica, e o tempo de transição aplicado na entrada da porta. Nas bibliotecas de célula atuais, o modelo não linear de atraso e transição é definido por \emph{lookup tables} como a mostrada na Figura \ref{fig:lookup_table}.
...
\end{figure}
		...
		O tempo de chegada, em um pino de entrada $j$ de uma porta lógica $i$ calculado da seguinte maneira:
		...
		O \emph{slew}, em um pino de entrada $j$ de uma porta lógica $i$ e definido por:
...
\chapter{Implementação da Ferramenta de Análise de Timing}
	...
	\section{Modelo de Grafo}
		...
		As estruturas de dados utilizadas para armazenar os elementos do grafo são essencialmente listas ordenadas topologicamente. À esquerda de um elemento, necessariamente se encontram os elementos de mesmo ou menor nível lógico, e à direita, de nível igual ao maior, como na Figura \ref{fig:grafo_lista_nivel_logico}. A mesma coisa acontece com os \emph{timing arcs} e com as \emph{timing nets}. Com essa escolha, o algoritmo de análise de timing estática passa a ser apenas de uma varredura em ordem, na lista, atualizando a informação de \emph{timing} acumulada para cada vértice do grafo.
	...
	\section{Algoritmo de Análise de Timing Estática}
		...
		\begin{algorithm}[H]
			...
			\Input{Grafo de \emph{timing} $G(V, E)$}
			\Output{Informações de \emph{timing} para os elementos do grafo (\emph{timing points, timing nets} e \emph{timing arcs})}
		...
		\begin{algorithm}[H]
			...
			\uIf{ $v_i$ é um pino de entrada ou saída primária }
			{
			...
			\ElseIf {$v_i$ é um pino de saída ou entrada primária}
			{
