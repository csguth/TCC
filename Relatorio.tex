...
Com o crescimento na dimensão dos circuitos digitais contemporâneos\footnote{Um processador para \textit{desktop} desenvolvido no ano de 2008 tem cerca de 731 milhões de transistores, excluindo a área de memória \cite{Intel08}.} e a necessidade de um tempo de entrega ao mercado (\textit{time-to-market}) curto faz com que esses projetos adotem o fluxo de projeto \textit{standard cell}.

No fluxo \textit{standard cell} as portas lógicas são desenvolvidas e verificadas previamente em uma dada tecnologia. Essas células, ou implementações disponíveis para as portas lógicas, são catalogadas com suas diversas características elétricas e tabelas (\textit{lookup tables}) de atraso em uma biblioteca de células, sendo reutilizadas em diversos projetos na mesma tecnologia. O reuso amortiza o custo dos projetos inseridos neste nodo tecnológico e \afazer{possibilta} um \textit{time-to-market} mais curto.

Os projetos de circuitos digitais no fluxo \textit{standard cell} são realizados visando, além das funcionalidades do projeto, a operação em uma frequência especificada. Por isso, diversas otimizações são efetuadas ao longo do fluxo, para que todos os recursos e tarefas do dispositivo consigam ser concluídas na frequência definida. Nas primeiras etapas de um projeto no fluxo \textit{standard cell}, apenas as questões relacionadas à funcionalidade do \textit{design} são verificadas, pois ainda, não se tem com precisão, as informações de atraso e informações elétricas necessárias, para realizar uma avaliação precisa de desempenho. 
...
No fluxo \textit{standard cell}, a partir do nível de transferência entre registradores (\textit{RTL: Register Transfer Level}), diversas otimizações como \textit{gate sizing} \cite{Livramento13} \cite{Hu12} \cite{Mohammad12} e \textit{Placement} \cite{Shahookar91} são realizadas. Como consequência dessas otimizações, as topologias das interconexões se alteram, necessitando ao decorrer do fluxo, diversas avaliações de suas informações temporais. Nas etapas iniciais, a análise de \textit{timing} pode ser requisitada milhares de vezes, demandando um baixo tempo de execução por parte desta. \afazer{\textbf{MUITO RUIM ISSO AQUI: }Por outro lado, devido ao fato de que as informações físicas do circuito, como de posicionamento, ainda são muito vagas, não espera-se que a qualidade dos resultados seja realista. Já nas etapas finais, mais próximas da síntese física, é necessário que a avaliação de \textit{Timing} considere o circuito no maior grau de detalhe possível, para que informações precisas sejam fornecidas. Essa demanda de precisão faz com que o tempo de simulação seja maior, o que já era esperado.}
...
% \includegraphics{sta.png}
\caption{Modelo de Capacitância Concentrada (\textit{Lumped C Model}) \cite{Rabaey08}.}
...
% \includegraphics{sta.png}
\caption{Representações utilizadas para as árvores RC em um contexto de \textit{pre-layout} \cite{BhaskerChadha09}.}
...
No fluxo \textit{Standard Cell}, as bibliotecas de célula restringem ao projetista, o uso de um número limitado de opções de implementação, para cada porta lógica do circuito. Também são impostos tempos máximos de transição e de capacitância que um pino de saída de uma porta pode carregar.
...
\item V  = \{ $v_i | v_i$ é um \textit{timing point} (pino de \textit{timing}), que pode ser um pino, uma entrada ou uma saída primária do circuito \}

\item I = \{ $(v_i, v_j) | v_i, v_j \in V \land (v_i, v_j)$ é uma interconexão do circuito, que conecta $v_i$ em $v_j$. $v_i$ é um pino de saída de uma porta lógica ou uma entrada primária, e $v_j$, pode ser a entrada de uma porta lógica ou uma saída primária. \}

\item A = \{ $(v_i, v_j) | v_i, v_j \in V \land (v_i, v_j)$ é um \textit{timing arc}. Portanto, $v_i$ e $v_j$ são pinos de entrada e saída (respectivamente) de uma mesma porta lógica. \}
...
\begin{figure}[ht]
\begin{center}% TRIM = L D R U
\includegraphics[width=1.1 \linewidth, trim=2cm 10cm 0 0]{imagens/comparacoes/primetime.pdf}
\caption{XXXXXXXXXXXXXXXXXXXX}
\label{fig:comparacoes_primetime}
\end{center}
\end{figure}

\begin{figure}[ht]
\begin{center}% TRIM = L D R U
\includegraphics[width=0.65 \linewidth, trim=4.5cm 20cm 9cm 2cm]{imagens/comparacoes/lump_elmore_slew.pdf}
\caption{XXXXXXXXXXXXXXXXXXXX}
\label{fig:lump_elmore_slew}
\end{center}
\end{figure}

\begin{figure}[ht]
\begin{center}% TRIM = L D R U
\includegraphics[width=1.1 \linewidth, trim=2cm 10cm 0 0]{imagens/comparacoes/lump_elmore_no_slew.pdf}
\caption{XXXXXXXXXXXXXXXXXXXX}
\label{fig:lump_elmore_no_slew}
\end{center}
\end{figure}

\begin{figure}[ht]
\begin{center}% TRIM = L D R U
\includegraphics[width=1.1 \linewidth, trim=2cm 10cm 0 0]{imagens/comparacoes/ceff_elmore_slew.pdf}
\caption{XXXXXXXXXXXXXXXXXXXX}
\label{fig:ceff_elmore_slew}
\end{center}
\end{figure}

\begin{figure}[H]
\begin{center}% TRIM = L D R U
\includegraphics[width=\linewidth, trim=0cm 10cm 2 0]{imagens/comparacoes/ceff_elmore_no_slew.pdf}
\caption{XXXXXXXXXXXXXXXXXXXX}
\label{fig:ceff_elmore_no_slew}
\end{center}
\end{figure}


Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla tincidunt dapibus mattis. Fusce sit amet dui in turpis aliquam feugiat in in dolor. Vestibulum hendrerit lacinia aliquam. Cras eleifend leo eget erat ultrices, vitae mattis neque consequat. Nullam eget laoreet mauris, sit amet lacinia diam. Phasellus id scelerisque quam. Mauris sollicitudin tincidunt accumsan. Mauris ipsum leo, eleifend nec aliquet ut, volutpat ut lectus. Donec eleifend magna nunc, tristique elementum justo condimentum eu. Curabitur vel nisl et massa euismod sollicitudin et sit amet enim. Quisque cursus porttitor pharetra. Donec non arcu nec ligula interdum luctus sit amet a turpis. Quisque porttitor, dui vitae euismod euismod, est elit suscipit ligula, et interdum mi elit ac dolor. Cras porttitor leo nisl, vel vulputate dolor vestibulum ut. Cras tincidunt quam at ante posuere rutrum non a dolor. Sed aliquam porttitor velit, vitae vestibulum justo tincidunt eget. Cras tincidunt quam at ante posuere rutrum non a dolor. Sed aliquam porttitor velit, vitae vestibulum justo tincidunt eget.  Cras tincidunt quam at ante posuere rutrum non a dolor. Sed aliquam porttitor velit, vitae vestibulum justo tincidunt eget.
