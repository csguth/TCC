...
\chapter{Introdução}
	...
	\section{Fluxo de Projeto \textit{Standard Cell}}
		...
		No Fluxo de Projeto \textit{Standard Cell} as células são desenvolvidas e verificadas previamente em uma dada tecnologia. Essas células, ou implementações disponíveis para as portas lógicas, são catalogadas com suas diversas características elétricas e tabelas (\textit{Lookup Tables}) de atraso em uma espécie de biblioteca, sendo reutilizadas em diversos projetos na mesma tecnologia. O reuso amortiza o custo dos projetos inseridos neste nodo tecnológico e possibilta um tempo de entrega ao mercado (\textit{time-to-market}) mais curto. Este é um dos principais motivos que faz com que os projetos dos dispositivos eletrônicos, como os \textit{PMDs} citados anteriormente, sigam o Fluxo \textit{Standard Cell}.
		...
		A partir das otimizações em nível de Transferência entre Registradores (\textit{RTL: Register Transfer Level}), as informações de desempenho começam a ser levantadas utilizando modelos de atrasos simplificados e pessimistas. Exemplos dessas otimizações são: \textit{Transistor Sizing}\cite{Fishburn85}, \textit{Gate Sizing}\cite{Livramento13}\cite{Hu12}\cite{Mohammad12}, \textit{Placement} \cite{Shahookar91}, entre outras. Ao decorrer do projeto, após a execução de variados processos de otimização, além do mapeamento do circuito na Biblioteca \textit{Standard Cell}, informações de posicionamento, interconexões e seus parasitas são obtidas, tornando possível e necessária, uma análise minuciosa das informações temporais, ou então, a chamada Análise de \textit{Timing}.
	...
	\section{Motivação}
		...
		\afazer{
			...
			Talvez falar em uma frase dos PMDs
			
			No Fluxo:
			\begin{itemize}

				\item As etapas finais (\textit{sign-off}) pedem maior precisão
				
				\item As etapas iniciais pedem velocidade, por causa das otimizações
		...
		A dimensão, e a complexidade, dos circuitos digitais contemporâneos, impedem que seus projetos sejam realizados manualmente, devido ao fato de que, com a evolução da tecnologia e das necessidades dos usuários, mais e mais recursos são integrados em uma mesma quantidade de silício. Assim, no projeto dos circuitos digitais atuais, é imprescindível o uso de ferramentas eficientes e escaláveis, para a Automação de Projeto Eletrônico (\textit{EDA: Eletronic Design Automation}).

	\section{Justificativa}

		Diversas otimizações são realizadas no decorrer do Fluxo de Projeto \textit{Standard Cells}, e o uso de ferramentas de EDA é indispensável em suas diferentes etapas. A inexistência de ferramentas de Análise de \textit{Timing} Estática precisas de domínio público, resulta em um problema de infraestrutura de pesquisa. Assim, este trabalho será uma alternativa de ferramenta de Análise de \textit{Timing} para projetistas de Circuitos Digitais, bem como uma infraestrutura realista e precisa, para desenvolvedores de ferramentas, que necessitam da Análise de \textit{Timing}, em alguma etapa do fluxo de projeto \textit{Standard Cells}.
...
\afazer{Fazer um link: Motivação: Primeiro parágrafo de introdução do paper ``Performance Optimization of VLSI Interconnect Layout''}
	...
	\subsection{Modelo RC Distribuído (\textit{Distributed RC Model})}
		...
		Como visto anteriormente, o Modelo RC Concentrado é uma a\-pro\-xi\-ma\-ção simplificada. O modelo de atraso de Elmore pode obter erros, pois ignora o efeito conhecido como \textit{resistive shield}, sendo impreciso quando se trata de interconexões grandes.

		\citeonline{Kashyap00} propuseram uma métrica, que leva em consideração o efeito de \textit{resistive shield}, calculando uma Capacitância Efetiva ($C_{eff}$) para modelar o impacto das capacitâncias \textit{downstream}.

		Devido a limitação de que o cálculo da Capacitância Efetiva depende do \textit{slew} aplicado no nodo fonte da Árvore RC, e que por sua vez, depende da Capacitância Efetiva, este trabalho abordará uma alternativa, proposta por \citeonline{PURI02}, computacionalmente eficiente e precisa, para estimativa dos atrasos das interconexões em circuitos \textit{CMOS}, contornando o problema observado em por \citeonline{Kashyap00}.
%
%		\subsubsection{Desafios de Implementar o Modelo RC Distribuído}
%
%			\afazer{
...
%			}
	...
	\subsection{Modelo de Capacitância Concentrada (\textit{Lumped C Model})}
		...
		O Modelo \textit{Lumped} foi amplamente utilizado, por se tratar de um modelo simplista que provê baixo tempo de execução. Ele assume que a resistência da interconexão é despresível. Geralmente, isso é verdade, quando a resistência da porta lógica que se conecta no fio, ou \textit{driver}, é substancialmente maior que a resistência da interconexão.

		Como a resistência da interconexão é despresível, ela pode ser representada como um capacitor e seu tempo de propagação pode ser desconsiderado como impacto global no circuito. O único impacto no desempenho é devido ao tempo de carga deste capacitor pela porta lógica \textit{driver} da interconexão \cite{Rabaey08}.
		...
		\subsubsection{Limitações do Modelo Concentrado}
			...
			Em muitos sistemas desenvolvidos, cerca de 50\% a 70\% do ciclo de relógio é \"consumido\" pelo atraso de propagação de suas interconexões \cite{Cong96}. Nas tecnologias com alta escala de integração (\textit{VLSI: Very-large-scale integration}) atuais, onde diversas otimizações tem por objetivo reduzir a resistência dos \textit{drivers} dos fios, estes passam a ser impactantes no desempenho do circuito digital, necessitando de uma representação mais afiada de seus modelos de atraso, levando em conta também, as suas resistências.

	\subsection{Modelo RC Concentrado (\textit{Lumped RC Model})}

		\afazer{n~ao falar do Elmore}

		Uma das formas de se representar as resistências e as capacitâncias como parasitas é o Modelo RC Concentrado (\textit{Lumped RC Model}). No Modelo RC Concentrado, concentra-se toda a resistência de cada parte da interconexão em um $R$ e similarmente combina-se a capacitância total em um único capacitor $C$. É um modelo simplificado \cite{Cong96}, pessimista \cite{Gupta97}, e pode ser ineficiente para cálculo de atraso em interconexões muito grandes.

		Considerando a rede resistor-capacitor ilustrada na Figura \ref{fig:rc_tree}. Esse circuito é chamado de Árvore RC (\textit{RC Tree}) e tem as seguintes propriedades:
		\begin{itemize}
...
Interconnect trees extracted from pre-layout or post-layout... Best-case, Balanced tree, worst-case		
	...
	%\includegraphics[width=0.8 \linewidth]{imagens/methodology_for_rc_delay_calculation.png} 
		...
		Considerando o inversor da Figura \ref{fig:arco_de_timing_fall_rise_slew_delay}, seu arco de \textit{timing} é representado pela conexão entre o pino de entrada $a$ e o pino de saída $o$. O atraso de propagação (\textit{delay}) é o tempo que o sinal no pino $o$ leva para se estabilizar, devido a uma mudança no sinal no pino $a$. Se a transição na saída for de 0 para Vdd, o atraso é chamado de atraso de subida (\textit{rise delay}), caso o contrário, é chamado de atraso de descida (\textit{fall delay}). O tempo de transição (\textit{slew}), é o tempo que o sinal leva para mudar de nível lógico. Se for uma mudança positiva, a transição é chamada de transição de subida (\textit{rise slew}), caso contrário, transição de descida (\textit{fall slew}). Com o objetivo de simplificar as futuras apresentações, somente um atraso e um \textit{slew} serão considerados.
