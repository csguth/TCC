...
\chapter{An\'alise de \emph{Timing} Estática}
	...
	\section{Representação de Circuitos Digitais}
		...
		Um par de vértices, $v_i$ e $v_j \in V$ são conectados por uma aresta direcionada $a(v_i, v_j) \in A$ se existe uma interconexão da saída do elemento depresentado pelo vértice $v_i$ para a entrada do elemento representado pelo vértice $v_j$. Um exemplo de circuito e seu grafo correspondente são ilustrados nas Figuras \ref{fig:exemplo_circuito_simple} e \ref{fig:exemplo_grafo_simple}. Node que neste caso, a entrada primária de \emph{clock} e sua interconexão não é representada no modelo de grafo, visto que neste trabalho, a abordagem será restringida a análise de timing em circuitos combinacionais, ou seja, não serão considerados tempos de \emph{setup}, \emph{hold}.
		...
		\begin{figure}[ht]
		\begin{center}
		\includegraphics[width=\linewidth, trim=0 8cm 0 8cm]{imagens/exemplo_grafo_simple.pdf}
		% \includegraphics{sta.png}
