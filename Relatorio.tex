...
\begin{itemize}
	\item \textbf{Atraso de propagação (\textit{delay}): } é o tempo que o sinal em um pino de saída $o$ leva para atingir 50\% de sua transição total, devido a uma mudança no sinal em um pino de entrada $a$. Se a transição em $o$ for do nível lógico 0 para 1, o atraso é chamado de atraso de subida (\textit{rise delay}), caso o contrário, é chamado de atraso de descida (\textit{fall delay}).
	
	\item \textbf{Tempo de Transição (\textit{Slew): }} é o tempo que o sinal leva para transicionar de uma tensão de referência a outra, por exemplo: $0 \to 5V$. Se for uma mudança positiva, a transição é chamada de transição de subida (\textit{rise slew}), caso contrário, transição de descida (\textit{fall slew}). 
	
	\item \textbf{Propagação do \textit{Slew: }}
	


\end{itemize}

Com o objetivo de simplificar as futuras apresentações, somente um atraso e um \textit{slew} serão considerados.
