...
No fluxo \textit{standard cell}, a partir do nível de transferência entre registradores (\textit{RTL: Register Transfer Level}), uma série otimizações são realizadas ao decorrer de suas etapas. Como consequência dessas otimizações, as topologias das interconexões se alteram, necessitando ao decorrer do fluxo, diversas avaliações de suas informações temporais.

Durante as otimizações nas etapas iniciais do fluxo (\textit{pre-layout}), a análise de \textit{timing} é requisitada milhares de vezes, assim, é necessário que a ferramenta de análise de \textit{timing} realize a análise de \textit{timing} com o melhor desempenho possível. Como as informações relacionadas ao aspecto físico do circuito, como posicionamento \afazer{CITAR} e roteamento \afazer{CITAR} ainda não são definidas com precisão nas etapas iniciais, essas informações são aproximadas e a ferramenta de análise de \textit{timing} fornece dados pessimistas sobre o \textit{timing} do circuito.
...
\begin{table}[ht]
\begin{center}% TRIM = L D R U
\includegraphics[width=\textwidth, trim=2cm 20cm 5cm 3cm]{imagens/comparacoes/lumped_capacitance_vs_primetime.pdf}
\caption{Comparação dos das informações de \textit{timing} obtidas com a ferramenta \textit{vs.} informações fornecidas pelo \textit{PrimeTime}, utilizando o modelo de interconexões de capacitância concentrada. O erro relativo é calculado utilizando a seguinte fórmula: $erro = \frac{|x - x*|}{\max(|x|, |x*|)}$, onde $x$ é a informação obtida pela ferramenta, e $x*$ é a informação fornecida pelo PrimeTime. \afazer{Falar no texto que é arrival, slack e slew.}}
\label{tab:lumped_capacitance_vs_primetime}
\end{center}
\end{table}

\textbf{Justificativa para o erro alto no benchmark \textit{cordic}:}

Nos pinos $g744540:a$, $g744569:o$, $g744569:a$, $g744621:o$ e $g744621:b$, a ferramenta reporta $0.00397127$ para \textit{slack} contra $0.004517$ do \textit{PrimeTime}. Aplicando $erro = \frac{|0.00397127 - 0.004517|}{\max(|0.00397127|, |0.004517|)}$, obtém-se $erro = 0.1208169139$, porém com $|0.00397127 - 0.004517|$, conclui-se que as duas ferramentas fornecem valores iguais até a $3^{\underline{a}}$ casa decimal, pelo menos.
...
\caption{$pci\_bridge32$ \textit{slew}}
\end{figure}
